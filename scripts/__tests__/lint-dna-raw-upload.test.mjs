/**
 * scripts/__tests__/lint-dna-raw-upload.test.mjs
 *
 * Unit tests for `scripts/lint-dna-raw-upload.mjs`.
 */
...
const SCRIPT = "scripts/lint-dna-raw-upload.mjs";
...
describe("lint-dna-raw-upload", () => {
  let tmpDir;

  before(() => {
    tmpDir = mkdtempSync(join(tmpdir(), "lint-dna-raw-"));
  });
  ...
  it("passes when the canonical contract is valid", () => {
    ...
    if (!result.stdout.includes("E10.4 DNA raw upload + matching policy contract OK")) {
