/**
 * scripts/__tests__/lint-entitlements.test.mjs
 *
 * Unit tests for `scripts/lint-entitlements.mjs`.
...
const SCRIPT = "scripts/lint-entitlements.mjs";
...
describe("lint-entitlements", () => {
  let tmpDir;
  before(() => {
    tmpDir = mkdtempSync(join(tmpdir(), "lint-ent-"));
  });
  after(() => {
  ...
  it("passes when the canonical contract is valid", () => {
    ...
    if (!result.stdout.includes("E11.4 Entitlement / quota / billing contract OK")) {
