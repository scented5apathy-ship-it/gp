/**
 * scripts/__tests__/lint-notifications.test.mjs
 *
 * Unit tests for `scripts/lint-notifications.mjs`.
...
const SCRIPT = "scripts/lint-notifications.mjs";
...
describe("lint-notifications", () => {
  let tmpDir;
  before(() => {
    tmpDir = mkdtempSync(join(tmpdir(), "lint-notif-"));
  });
  after(() => {
  ...
  it("passes when the canonical contract is valid", () => {
    ...
    if (!result.stdout.includes("E11.1 Notification policy contract OK")) {
