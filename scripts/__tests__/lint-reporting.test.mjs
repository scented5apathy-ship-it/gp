/**
 * scripts/__tests__/lint-reporting.test.mjs
 *
 * Unit tests for `scripts/lint-reporting.mjs`.
...
const SCRIPT = "scripts/lint-reporting.mjs";
...
describe("lint-reporting", () => {
  let tmpDir;
  before(() => {
    tmpDir = mkdtempSync(join(tmpdir(), "lint-rep-"));
  });
  after(() => {
  ...
  it("passes when the canonical contract is valid", () => {
    ...
    if (!result.stdout.includes("E11.3 Reporting policy contract OK")) {
