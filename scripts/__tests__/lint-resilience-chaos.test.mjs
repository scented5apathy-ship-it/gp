/**
 * scripts/__tests__/lint-resilience-chaos.test.mjs
 *
 * Unit tests for `scripts/lint-resilience-chaos.mjs` (E13.4).
 */
...
const SCRIPT = "scripts/lint-resilience-chaos.mjs";
...
describe("lint-resilience-chaos", () => {
  it("passes when the canonical contract is valid", () => {
    ...
    if (!result.stdout.includes("E13.4 summary: OK")) {
  ...
  it("rejects when faultClasses closed-set is missing an entry", () => {
    ...
    try {
      const tampered = backup.replace(
        "    - name: pod_kill\n",
        "    # removed: - name: pod_kill\n",
      );
      writeFileSync(contractPath, tampered);
      const result = runLinter({});
      if (result.status === 0) {
        throw new Error("linter accepted missing fault class");
      }
      if (!/faultClasses/.test(combined(result))) {
        throw new Error(`missing fault class message:\n${combined(result)}`);
      }
    } finally {
  ...
  it("rejects when retry policy exceeds budget", () => {
    const contractPath = resolve(
      "contracts/reliability/resilience-chaos-policy.yaml",
    );
    const backup = readFileSync(contractPath, "utf8");
    try {
      ...
      const result = runLinter({});
      if (result.status === 0) {
        throw new Error("linter accepted over-budget retry");
      }
      if (!/retryPolicies/.test(combined(result))) {
