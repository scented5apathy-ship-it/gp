/**
 * scripts/__tests__/lint-slo-alert.test.mjs
 *
 * Unit tests for `scripts/lint-slo-alert.mjs` (E13.2).
 */
...
const SCRIPT = "scripts/lint-slo-alert.mjs";
...
describe("lint-slo-alert", () => {
  it("passes when the canonical contract is valid", () => {
    ...
    if (!result.stdout.includes("E13.2 summary: OK")) {
  ...
  it("rejects when the SLI closed-set is missing an entry", () => {
    ...
    try {
      const tampered = backup.replace(
        ...
        "    - name: api_read_p95\n",
        "    # removed: - name: api_read_p95\n",
      );
      writeFileSync(contractPath, tampered);
      const result = runLinter({});
      if (result.status === 0) {
        throw new Error("linter accepted missing SLI");
      }
      if (!/slis/.test(combined(result))) {
        throw new Error(`missing slis message:\n${combined(result)}`);
      }
    } finally {
  ...
  it("rejects when the budgetStateMatrix terminal has transitions", () => {
    const contractPath = resolve(
      "contracts/reliability/slo-alert-policy.yaml",
    );
    const backup = readFileSync(contractPath, "utf8");
    try {
      ...
      if (!/budgetStateMatrix/.test(combined(result))) {
        throw new Error(`missing budget matrix message:\n${combined(result)}`);
      }
    } finally {
      writeFileSync(contractPath, backup);
    }
