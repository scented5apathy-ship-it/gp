/**
 * E6.2 + E6.3 collaboration-service domain aggregates + value objects.
 *
 ...
 * R10.6 (approved change traces back to proposal + reviewer),
 * `design.md` §8.3 (base version + normalized patch/command,
 * no arbitrary JSON patch on forbidden fields, OpenFGA + ABAC
 * re-check at approve time, merge produces a new domain command)
 * and ADR-E0.5-08 (BACKWARD evolution of closed-set
 * vocabularies).
 *
 ...
 * and {@code FlagsmithRolloutSync} for the Flagsmith safe-default
 * rollout sync (E6.3).
 *
