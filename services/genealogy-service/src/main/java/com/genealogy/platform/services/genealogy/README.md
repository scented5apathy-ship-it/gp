...
Cross-service package marker. The `com.genealogy.platform.services.genealogy`
main source directory inside `services/genealogy-service` is intentionally empty
...
- No file under `services/genealogy-service/` may import another service's
  ...
  package — including the `genealogy` package.
...
Owner of the `genealogy` package: see `services/genealogy/OWNERS`.
