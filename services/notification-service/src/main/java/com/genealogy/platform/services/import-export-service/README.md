# services/notification-service → import-export-service

Cross-service package marker. The `com.genealogy.platform.services.import-export-service`
main source directory inside `services/notification-service` is intentionally empty
in the E1.1 scaffold. It exists so the ArchUnit boundary guard
(`AGENTS.md` §2 / `scripts/check-monorepo-boundaries.mjs`) can
detect any future drift where `services/notification-service` accidentally
...
- No file under `services/notification-service/` may import another service's
  ...
  package — including the `import-export-service` package.
...
Owner of `services/notification-service`: see `services/notification-service/OWNERS`.
Owner of the `import-export-service` package: see `services/import-export-service/OWNERS`.
