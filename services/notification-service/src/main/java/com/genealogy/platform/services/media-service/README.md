# services/notification-service → media-service

Cross-service package marker. The `com.genealogy.platform.services.media-service`
...
- No file under `services/notification-service/` may import another service's
  ...
  package — including the `media-service` package.
...
Owner of the `media-service` package: see `services/media-service/OWNERS`.
