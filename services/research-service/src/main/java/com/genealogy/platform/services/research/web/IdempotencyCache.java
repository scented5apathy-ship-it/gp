 ...
 * replays the stored response when the same key arrives within
 * the window.
 *
 * <p>The cache lives in memory only — durable idempotency lands
 * in E15 once the dedicated Redis/Valkey backend is available.
 * The default 24-hour window matches the value documented in
 * the OpenAPI header definition.
 *
 * <p>Thread-safety: every entry is guarded by a {@code synchronized}
 * block on the entry reference; the outer map is a
 * {@link ConcurrentHashMap} so reads do not block on writes. The
 * TTL sweep is best-effort — an expired entry may survive a few
 * minutes past its deadline; the controller accepts the expired
 * replay (the original response is still correct) and the
 * {@link #store} call always overwrites.
 */
@Component
public class IdempotencyCache {
     ...
     * status code (so 201, 200, 412 are preserved exactly); the
