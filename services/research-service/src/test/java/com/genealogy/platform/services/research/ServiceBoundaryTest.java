...
/**
 * Cross-service boundary guard for {@code research-service}.
 *
 * <p>This test enforces the E1.1 acceptance criterion: a service
 * must NOT import another service's {@code db/} (Flyway / jOOQ)
 * or {@code domain/} (entities, value objects) package.
 * Violations cause the build to fail.
 *
 * <p>{@code .allowEmptyShould(true)} is set so the rule does not
 * fail while the service has zero production classes (E1.1
 * skeleton phase); the rule becomes binding as soon as E3.x /
 * E6.x adds the research aggregate root and friends.
 */
@AnalyzeClasses(
    packages = "com.genealogy.platform.services.research",
...
class ServiceBoundaryTest {
    ...
    static final ArchRule SERVICES_MUST_NOT_IMPORT_OTHER_SERVICES_DB =
        ArchRuleDefinition.noClasses()
            .that().resideInAPackage("com.genealogy.platform.services.research..")
